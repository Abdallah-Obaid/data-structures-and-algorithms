...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((count,ele) =>{
    var arr = ele.filter(inele =>{
      return inele === target
    });
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((count,ele)=>{
    return count = count + ele.reduce((incount,inele) =>{
      return incount = incount + inele; 
    },0)
  },0)
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(ele =>{
    return ele.reduce((incount,inele) =>{
      if(typeof(inele) === "number" && inele%5 === 0){
        incount.push(Math.pow(2,inele))
      }
      return incount
    },[])
  })
