...
const reversedString = (str) => {
  ...
  var arr = str.split("")
  return arr.reduce((count,ele,index)=>{
    count=count+str[str.length-1-index]
    return count;
  },"")
};
...
const countNumberOfChildren = (arr) => {
  ...
  return arr.reduce((count,ele,index)=>{
    if(ele.children){
    count = count + ele.children.length
  }
  return count;
},0);
}
...
const calculateAverage = (arr) => {
  // Solution code here...
  var obj = arr.reduce((total,ele)=>{
    total.count = total.count + 1;
    total.sum = total.sum + ele;
    return total;
  },{ count: 0, sum: 0 });
...
const extractStat = (statName, arr) => {
  // Solution code here...
  var obj= arr.reduce((count,ele,index)=>{
    if (ele.stat.name === statName){
      return ele
    }
    return count;
  },{})
  console.log(obj,statName)
  if (obj === {}){
    return null;
  }else{
    return obj
...
const extractChildren = (arr) => {
  // Solution code here...
 var arrray= arr.filter(ele=>{
    return ele.name.include('a');
  })
  return arrray.reduce((count,ele,index)=>{
    if(ele.children){
    count.concat(ele.children)
    }
    return count;
